...
def main():
    ...
    with ImageInference(card_object_detection.model()) as inference:
        image = Image.open(args.input)
        image_center, offset = crop_center(image)

        if args.sparse:
            result = inference.run(image_center,
                                   sparse_configs=card_object_detection.sparse_configs(args.threshold))
            objects = card_object_detection.get_objects_sparse(result, offset)
        else:
            result = inference.run(image_center)
            objects = card_object_detection.get_objects(result, args.threshold, offset)
